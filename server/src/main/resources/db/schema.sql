...
CREATE TABLE IF NOT EXISTS t_api_definition (
    ...
    path_params       JSON NOT NULL COMMENT '路径参数 [{key,value,description}]',
    headers           JSON NOT NULL COMMENT '请求头 [{key,value,enabled,description}]',
    query             JSON NOT NULL COMMENT 'Query [{key,value,enabled,description,required}]',
    body_type         VARCHAR(32) NOT NULL DEFAULT 'NONE' COMMENT 'NONE/FORM_DATA/X_WWW_FORM_URLENCODED/JSON/XML/BINARY',
    body_schema       JSON NOT NULL COMMENT '请求体 Schema',
    body              LONGTEXT COMMENT '请求体示例',
    response_examples JSON NOT NULL COMMENT '按状态码 { "200": {...}, "400": {...} }',
    ...
    mock_status_code  INT NOT NULL DEFAULT 200,
    mock_response     LONGTEXT COMMENT 'BASIC 类型响应体',
    mock_rules        JSON NOT NULL COMMENT 'CONDITIONAL 类型规则 [{when,response,status}]',
    mock_script       LONGTEXT COMMENT 'DYNAMIC 类型 JS 脚本',
    source_url        VARCHAR(512) COMMENT 'Swagger 来源 URL',
    source_hash       VARCHAR(64) COMMENT '来源内容 SHA-256（同步差异用）',
    deleted_at        DATETIME COMMENT '软删除时间',
    created_at        DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at        DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    KEY idx_project (project_id),
    KEY idx_module (module_id),
    KEY idx_source (source_url)
